class mapmanager():
    ...
    def isEmply(self, pos):
        blocks = self.findBlocks(pes)
        if blocks:
            return False
        else:
            return True
    ...
    def findHighestEmpty(self, pos):
        x, y, z = posz = 1 
        while not self.isEmply((x, y, z)):
            z += 1 
            return (x, y, z)
    
    def buildBlock(self, pos):
        #ставим блок с учеом гравитации
        x, y, z = pos
        new = self.findHighestEmpty(pos)
    
    def delBlock(self, position):
        #удаляем блок в указанной позиции
        blocks = self.findBlocks(position)
        for block in blocks:
            block.removeNode()
    def delBlockFrom(self, possition):
